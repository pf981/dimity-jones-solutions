...
def evaluate_fitness(freqs: Dict[str, float], reference_freqs: Dict[str, float]) -> float:
...
def find_key_length(ciphertext: str, max_length: int = 20) -> int:
    ...
    for length in range(1, min(max_length + 1, len(ciphertext) // 2)):
        ...
        total_ic = sum(calculate_index_of_coincidence(subseq) for subseq in subsequences)
...
def find_best_key_char(ciphertext: str, reference_trigrams: Dict[str, float], 
                      decrypter_func) -> Tuple[str, float]:
...
def get_reference_frequencies_by_position(reference_text: str, key_length: int, 
                                        decrypter_func) -> List[Dict[str, float]]:
...
def break_vigenere_cipher(ciphertext: str, reference_text: str, 
                         decrypter_func, max_key_length: int = 20, 
    ...
    position_references = get_reference_frequencies_by_position(reference_text, key_length, decrypter_func)
    ...
    for pos in range(key_length):
        ...
        if len(subseq) > 0:
            ...
            ref_trigrams = position_references[pos] if pos < len(position_references) else {}
            if not ref_trigrams:
...
def try_multiple_key_lengths(ciphertext: str, reference_text: str, 
                            decrypter_func, min_length: int = 1, 
                            max_length: int = 15, verbose: bool = True) -> List[Tuple[str, str, float]]:
    """
    ...
    for length in range(min_length, min(max_length + 1, len(ciphertext) // 3)):
        ...
        position_references = get_reference_frequencies_by_position(reference_text, length, decrypter_func)
