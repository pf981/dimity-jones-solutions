...
def number_to_english(n: int) -> str:
    ...
    elif n < 1_000 and n % 100 == 0:
        return number_to_english(n // 100) + "hundred"
    elif n < 1_000:
        return number_to_english(n // 100) + "hundredand" + number_to_english(n % 100)
    elif n < 1_000_000:
        return number_to_english(n // 1_000) + "thousand" + number_to_english(n % 1_000)
    elif n < 1_000_000_000:
        return (
            number_to_english(n // 1_000_000)
            + "million"
            + number_to_english(n % 1_000_000)
        )
    elif n < 1_000_000_000_000:
        return (
            number_to_english(n // 1_000_000_000)
            + "billion"
            + number_to_english(n % 1_000_000_000)
        )
...
def division_to_letter(match: re.Match[str]) -> str:
    a, b = match.groups()
    return number_to_english(int(b))[int(a) - 1]


def decode(text: str) -> str:
    return (
        re.sub(r"(\d+)/(\d+)", division_to_letter, text)
        .replace(" + ", "")
        .replace(",", "")
    )
...
def decrypt(cipher: str) -> str:
    return decrypter.vigenere_cipher(cipher, "gtn")
