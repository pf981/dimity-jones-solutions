...
len(pages)  # 9 - so clearly a 3x3 grid
# grid = ["" * 3 for _ in range(3)]

for i, (page, rips) in enumerate(pages):
    print(i, rips)
...
# 3, 7, 8
# Just put them anywhere for now
grid_indexes = [
    [2, 3, 7],
    [6, 0, 1],
    [4, 8, 5],
...
grid = []
for row_indexes in grid_indexes:
    lines_list = [pages[i][0].strip().splitlines() for i in row_indexes]
    print(lines_list)
    grid.append("\n".join("".join(line) for line in zip(*lines_list)))

combined = "\n".join(grid)
with open("data/88_pages.txt", "w") as f:
    f.write(combined)


combined_counts = collections.Counter(combined)
combined_counts

"".join(sorted(combined_counts))
# This uses all characters!

len(combined.splitlines())  # 70
len(combined.splitlines()[0])  # 107
